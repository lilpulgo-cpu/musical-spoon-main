...
def CohereAttention_fast_forward_inference(
    ...
    if do_prefill:
        ...
        if self.use_qk_norm:
            self.q_norm_out_weight = torch.empty(self.q_norm.weight.shape, dtype = torch.float32, device = "cpu")
            self.k_norm_out_weight = torch.empty(self.k_norm.weight.shape, dtype = torch.float32, device = "cpu")
        else:
